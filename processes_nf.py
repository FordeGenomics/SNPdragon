...
class Results:
    ...
    def addSample(self, sample_name, frac, cov_file, vcf_file):
        sample = Sample(sample_name, frac, cov_file, vcf_file)
        self.samples.append(sample)

    def getAllFiltered(self):
        """ Returns dict{contig:set{pos.alt}} of all high quality variants across all samples 
        Where pos.alt is a float """
        allVars = {}
...
class Mtx:
    ...
    def __init__(self, ref, sample, results, maskFile = None, threads = 1, recomb = True, exclude = False, outdir=""):
        ...
        # print(f"Memory start {current / 1024**2}MB; Peak was {peak / 1024**2}MB")
        
        self.__createRefFrame()

        # current, peak = tracemalloc.get_traced_memory()
        # print(f"Ref frame created {current / 1024**2}MB; Peak was {peak / 1024**2}MB")

        self.__buildMtx() # Matrix of all variant and low coverage positions

        # current, peak = tracemalloc.get_traced_memory()
        # print(f"Memory buildMtx {current / 1024**2}MB; Peak was {peak / 1024**2}MB")

        self.__getSnpMtx()

        # current, peak = tracemalloc.get_traced_memory()
        # print(f"Memory snpMtx {current / 1024**2}MB; Peak was {peak / 1024**2}MB")
        
        self.__getCoreMtx()

        # current, peak = tracemalloc.get_traced_memory()
        # print(f"Memory coreMtx {current / 1024**2}MB; Peak was {peak / 1024**2}MB")
    ...
    def writeFastaAlns(self):
        ...
        for contig in self.snp.keys():
            ...
            # # unthreaded
            # core_stats = []
            # for sample_obj in self.results.samples:
            #     sample = sample_obj.name
            #     core_stats.append(self.writeFastaHelper([sample, contig, full_length]))

            # # multi-threaded
            # write_fasta_args = []
            # for sample_obj in self.results.samples:
            #     sample = sample_obj.name
            #     write_fasta_args.append([sample, contig, full_length])
            # p = ThreadPool(processes=min(self.threads,len(self.results.samples)))
            # core_stats = p.map(self.writeFastaHelper, write_fasta_args)
            # p.close()
    ...
    def writeFastaHelper(self, args):
        ...
        core_length = len("".join(fasta_list[1:]).replace("-",""))
        stats_string = "{}\t{}\t{}\t{}\n".format(sample, contig, core_length, core_length/full_length*100)
