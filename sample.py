...
class Sample:
    ...
    def __init__(self, name, frac, cov_file, vcf_file, outdir="", filt_clust=False, filt_cliffs=True):
        ...
        # current, peak = tracemalloc.get_traced_memory()
        # print(f"read pileup file {current / 1024**2}MB; Peak was {peak / 1024**2}MB")

        self.readVCF(vcf_file)

        # current, peak = tracemalloc.get_traced_memory()
        # print(f"read vcf {current / 1024**2}MB; Peak was {peak / 1024**2}MB")

        if self.filter_clust:
            self.findClusters()

        # current, peak = tracemalloc.get_traced_memory()
        # print(f"clusters {current / 1024**2}MB; Peak was {peak / 1024**2}MB")
        ...
        # current, peak = tracemalloc.get_traced_memory()
        # print(f"build Series {current / 1024**2}MB; Peak was {peak / 1024**2}MB")
        ...
        # current, peak = tracemalloc.get_traced_memory()
        # print(f"cleanup {current / 1024**2}MB; Peak was {peak / 1024**2}MB")
    ...
    def readPileup(self, file):
        ...
        for contig in contigs:
            ...
            # low_cov = list(self.allCov[contig].loc[self.allCov[contig]["Depth"] < MIN_DEPTH].index) # Remove positions above depth
            # final_cov = []
            # for n in range(-2,3): # mask 5bp either side of large deletions
            #     final_cov.extend([max(i+n,0) for i in low_cov])
            # self.covSeries[contig] = list(set(sorted(final_cov)))
    ...
    def buildSeries(self):
        ...
        for contig in self.covSeries.keys(): # coverage may have contigs with no snps called
            ...
            # self.covSeries[contig] = pd.DataFrame(np.zeros((1,len(self.covSeries[contig])), dtype='uint8'), columns=self.covSeries[contig])
    ...
    def getLowCov(self, var_list, contig):
        ...
        # low_cov_cols = [varList[e] for e,x in enumerate(varList) if x.pos in covCols]
